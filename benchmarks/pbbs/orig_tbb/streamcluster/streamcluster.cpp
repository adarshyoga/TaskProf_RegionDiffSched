...
public:
  ...
  task *execute() {
    ...
    //recycle_as_continuation();
	  ...
	  spawn( *new( allocate_child() ) CenterTableCount(p, stride, points, work_mem));
      CenterTableCount &me = *new( allocate_child() ) CenterTableCount(0, stride, points, work_mem);
      //spawn(list);
      //is_continuation = 1;
      spawn_and_wait_for_all(me);
      //return &me;
...
public:
  ...
  task *execute() {
    ...
    //recycle_as_continuation();
      set_ref_count(NUM_DIVISIONS+1);
      for(p = 1; p < (NUM_DIVISIONS); p++ ) 
	  spawn( *new( allocate_child() ) FixCenter(p, stride, points, work_mem));
      //spawn(list);
...
public:
  ...
  task *execute() {
    ...
    //recycle_as_continuation();
      set_ref_count(NUM_DIVISIONS+1);
      for(p = 1; p < (NUM_DIVISIONS); p++ ) 
	spawn( *new( allocate_child() )  LowerCost(p, stride, points, x, work_mem, K));
      //spawn(list);
...
public:
  ...
  task *execute() {
    ...
    //recycle_as_continuation();
      set_ref_count(NUM_DIVISIONS + 1);
      for(p = 1; p < (NUM_DIVISIONS); p++ ) 
	spawn( *new( allocate_child() )  CenterClose(p, stride, points, work_mem, K, z));
      //spawn(list);
...
public:
  ...
  task *execute() {
    ...
    //recycle_as_continuation();
      set_ref_count(NUM_DIVISIONS + 1);
      for(p = 1; p < (NUM_DIVISIONS); p++ ) 
	spawn( *new( allocate_child() )  SaveMoney(p, stride, points, x, work_mem));
      //spawn(list);
