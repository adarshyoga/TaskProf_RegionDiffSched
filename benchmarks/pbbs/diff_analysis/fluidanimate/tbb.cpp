...
template <class T>
class WorkerLauncher: public tbb::t_debug_task {
  ...
  task *execute() {
    ...
    //recycle_as_continuation();
      ...
      for(int t=1;t<NUM_TASKS;t++)
        spawn( *new( tbb::task::allocate_child() ) T(t, i_, sz, ez), __FILE__, __LINE__  );
...
template<class T>
class GridLauncher: public tbb::t_debug_task {
  bool is_continuation;
  int num_grids;
public:
  ...
  task *execute() {
    ...
    //if(!is_continuation) {
    //recycle_as_continuation();
      set_ref_count(num_grids + 1);

      for(int i=1;i<num_grids;i++)
        spawn( *new( tbb::task::allocate_child() ) WorkerLauncher<T>(i) , __FILE__, __LINE__);
...
class ClearParticlesMTWorker: public tbb::t_debug_task {
  int tid;
  int pid;
  int start;
  int end;
...
class RebuildGridMTWorker: public tbb::t_debug_task {
  int tid;
  int pid;
  int start;
  int end;
...
class InitDensitiesAndForcesMTWorker: public tbb::t_debug_task {
  int tid;
  int pid;
  int start;
  int end;
...
class ComputeDensitiesMTWorker: public tbb::t_debug_task {
  int tid;
  int pid;
  int start;
  int end;
...
class ComputeDensities2MTWorker: public tbb::t_debug_task {
  int tid;
  int pid;
  int start;
  int end;
...
class ComputeForcesMTWorker: public tbb::t_debug_task {
  int tid;
  int pid;
  int start;
  int end;
...
class ProcessCollisionsMTWorker: public tbb::t_debug_task {
  int tid;
  int pid;
  int start;
  int end;
...
class ProcessCollisions2MTWorker: public tbb::t_debug_task {
  int tid;
  int pid;
  int start;
  int end;
...
class AdvanceParticlesMTWorker: public tbb::t_debug_task {
  int tid;
  int pid;
  int start;
  int end;
...
void AdvanceFrame()
{
  ...
  tbb::t_debug_task::spawn_root_and_wait(cp, __FILE__, __LINE__);

  GridLauncher<RebuildGridMTWorker> &rg = *new (tbb::task::allocate_root()) GridLauncher<RebuildGridMTWorker>(NUM_GRIDS);
  tbb::t_debug_task::spawn_root_and_wait(rg, __FILE__, __LINE__);
  ...
  tbb::t_debug_task::spawn_root_and_wait(pc2, __FILE__, __LINE__);
#endif
}
