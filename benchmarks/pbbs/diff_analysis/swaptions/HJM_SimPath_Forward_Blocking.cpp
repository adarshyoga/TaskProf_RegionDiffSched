...
struct ParallelB {
  ...
  void operator()(const tbb::blocked_range<int> &range, size_t thdId) const {
    ...
    for(b=begin; b!=end; b++) {
      for (j=1;j<=iN-1;++j){
	//RecordMem(thdId, &(pdZ[l][BLOCKSIZE*j + b]), WRITE);
...
//This function computes and stores an HJM Path for given inputs
	...
	for(l=0;l<=iFactors-1;++l){
	  B.set_l(l);
	  tbb::parallel_for(tbb::blocked_range<int>(0, BLOCKSIZE, PARALLEL_B_GRAINSIZE),B, tbb::simple_partitioner(), __FILE__, __LINE__);
	}
