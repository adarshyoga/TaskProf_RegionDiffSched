...
struct Worker {
  ...
  void operator()(const tbb::blocked_range<int> &range, size_t thdId) const {
    ...
    for(int i=begin; i!=end; i++) {
      ...
      assert(iSuccess == 1);
      //RecordMem(thdId, &(swaptions[i].dSimSwaptionMeanPrice), WRITE);
      swaptions[i].dSimSwaptionMeanPrice = pdSwaptionPrice[0];
      //RecordMem(thdId, &(swaptions[i].dSimSwaptionStdError), WRITE);
...
#ifdef TBB_VERSION
	Worker w;
	tbb::parallel_for(tbb::blocked_range<int>(0,nSwaptions,TBB_GRAINSIZE),w, tbb::simple_partitioner(), __FILE__, __LINE__);
#else
