...
public:
  ...
  void operator()(const tbb::blocked_range<int>& range, size_t thdid) {
    double myhiz = 0;
  ...
  void operator()(const tbb::blocked_range<int>&range, size_t thdid) const {
    ...
    int end = range.end();
     ...
     for( int k = begin; k!=end; k++ )    {
       ...
       //RecordMem(thdid, &points->p[k].weight, READ);
       //RecordMem(thdid, &points->p[k].cost, WRITE);
       points->p[k].cost = distance * points->p[k].weight;
       //RecordMem(thdid, &points->p[k].assign, WRITE);
       points->p[k].assign = 0;
     } 
  }
...
public:
  ...
  void operator()(const tbb::blocked_range<int> &range, size_t thdid) {
    ...
    if(type) {
      ...
      for(int k = begin; k!=end; k++ )  {
	//RecordMem(thdid, &points->p[k].cost, READ);
	local_total+=points->p[k].cost;
      }
      //RecordMem(thdid, &total_cost, WRITE);
...
class CenterTableCount: public tbb::t_debug_task{
private:
  Points *points;
  double *work_mem;
  int stride;
...
public:
  ...
  task *execute() {
    ...
    for( int i = k1; i < k2; i++ ) {
      if( is_center[i] ) {
	//RecordMem(get_cur_tid(), &center_table[i], WRITE);
	center_table[i] = count++;
      }
    }
    
    //RecordMem(get_cur_tid(), &work_mem[pid*stride], WRITE);
...
class CenterTableCountTask: public tbb::t_debug_task {
  int is_continuation;
  Points *points;
  double *work_mem;
  int stride;
public:
  ...
  task *execute() {
    ...
    //recycle_as_continuation();
      set_ref_count(NUM_DIVISIONS + 1);

      for(p = 1; p < (NUM_DIVISIONS); p++ ) 
	spawn( *new( allocate_child() ) CenterTableCount(p, stride, points, work_mem), __FILE__, __LINE__);
      CenterTableCount &me = *new( allocate_child() ) CenterTableCount(0, stride, points, work_mem);
      //spawn(list);
      //is_continuation = 1;
      spawn_and_wait_for_all(me, __FILE__, __LINE__);
      //return &me;
	...
	//RecordMem(get_cur_tid(), &work_mem[p*stride], READ);
	int tmp = (int)work_mem[p*stride];
	//RecordMem(get_cur_tid(), &work_mem[p*stride], WRITE);
...
class FixCenter: public tbb::t_debug_task {
  Points *points;
  double *work_mem;
  int pid;
  int stride;
public:
  ...
  task *execute(){
    ...
    for( int i = k1; i < k2; i++ ) {
      if( is_center[i] ) {
	//RecordMem(get_cur_tid(), &work_mem[pid*stride], READ);
	//RecordMem(get_cur_tid(), &center_table[i], WRITE);
...
class FixCenterTask: public tbb::t_debug_task {
...
public:
  ...
  task *execute() {
    ...
    //recycle_as_continuation();
      set_ref_count(NUM_DIVISIONS+1);
      for(p = 1; p < (NUM_DIVISIONS); p++ ) 
	spawn( *new( allocate_child() ) FixCenter(p, stride, points, work_mem), __FILE__, __LINE__);
      //spawn(list);
...
class LowerCost: public tbb::t_debug_task {
  Points *points;
  double *work_mem;
  long x;
  int K;
...
public:
  ...
  task *execute() {
    ...
    //RecordMem(get_cur_tid(), &points->num, READ);
	...
	* points->p[i].weight;
      //RecordMem(get_cur_tid(), &points->p[i], READ);
	...
	//RecordMem(get_cur_tid(), &points->p[i].assign, READ);
	int assign = points->p[i].assign;
	//RecordMem(get_cur_tid(), &center_table[assign], READ);
	//RecordMem(get_cur_tid(), &lower[center_table[assign]], READ);
	//RecordMem(get_cur_tid(), &lower[center_table[assign]], WRITE);
...
class LowerCostTask: public tbb::t_debug_task {
  bool is_continuation;
  Points *points;
  double *work_mem;
  int K;
...
public:
  ...
  task *execute() {
    ...
    //recycle_as_continuation();
      set_ref_count(NUM_DIVISIONS+1);
      for(p = 1; p < (NUM_DIVISIONS); p++ ) 
	spawn( *new( allocate_child() )  LowerCost(p, stride, points, x, work_mem, K), __FILE__, __LINE__);
      //spawn(list);
...
class CenterClose: public tbb::t_debug_task {
  Points *points;
  double *work_mem;
...
public:
  ...
  task *execute() {
	...
	if( is_center[i] ) {
	  ...
	  for( int p = 0; p < (NUM_DIVISIONS); p++ ) {
	    //RecordMem(get_cur_tid(), &center_table[i], READ);
	    //RecordMem(get_cur_tid(), &work_mem[center_table[i]+p*stride], READ);
...
class CenterCloseTask: public tbb::t_debug_task {
  bool is_continuation;
  Points *points;
  double *work_mem;
  int stride;
...
public:
  ...
  task *execute() {
    ...
    //recycle_as_continuation();
      set_ref_count(NUM_DIVISIONS + 1);
      for(p = 1; p < (NUM_DIVISIONS); p++ ) 
	spawn( *new( allocate_child() )  CenterClose(p, stride, points, work_mem, K, z), __FILE__, __LINE__);
      //spawn(list);
...
class SaveMoney: public tbb::t_debug_task{
  Points *points;
  double *work_mem;
  long x;
  int pid, stride;
public:
  ...
  task *execute() {
    ...
    for ( int i = k1; i < k2; i++ ) {
      //RecordMem(get_cur_tid(), &points->p[i].assign, READ);
      //RecordMem(get_cur_tid(), &center_table[points->p[i].assign], READ);
      //RecordMem(get_cur_tid(), &gl_lower[center_table[points->p[i].assign]], READ);
      bool close_center = gl_lower[center_table[points->p[i].assign]] > 0 ;
      if ( switch_membership[i] || close_center ) {
	// Either i's median (which may be i itself) is closing,
	// or i is closer to x than to its current median
	//RecordMem(get_cur_tid(), &points->p[i].weight, READ);
	//RecordMem(get_cur_tid(), &points->p[i].cost, WRITE);
    ...
    for( int i = k1; i < k2; i++ ) {
      //RecordMem(get_cur_tid(), &is_center[i], READ);
      //RecordMem(get_cur_tid(), &center_table[i], READ);
      //RecordMem(get_cur_tid(), &gl_lower[center_table[i]], READ);
...
class SaveMoneyTask: public tbb::t_debug_task {
  bool is_continuation;
  Points *points;
  long x;
  double* work_mem;
...
public:
  ...
  task *execute() {
    ...
    //recycle_as_continuation();
      set_ref_count(NUM_DIVISIONS + 1);
      for(p = 1; p < (NUM_DIVISIONS); p++ ) 
	spawn( *new( allocate_child() )  SaveMoney(p, stride, points, x, work_mem), __FILE__, __LINE__);
      //spawn(list);
...
float pspeedy(Points *points, float z, long *kcenter)
{
  ...
  /* create center at first point, send it to itself */
  {
    ...
    tbb::parallel_for(tbb::blocked_range<int>(0,points->num, grain_size),c, tbb::simple_partitioner(), __FILE__, __LINE__);
  }
  ...
  *kcenter = 1;
...

  {
	...
	tbb::parallel_reduce(tbb::blocked_range<int>(0,points->num,grain_size),c,tbb::simple_partitioner(), __FILE__,__LINE__);
      }
    }
	       ...
	       int pid, pthread_barrier_t* barrier )
{
  ...
  if(grain_size==0)
    ...
    tbb::parallel_reduce(tbb::blocked_range<int>(0,points->num, grain_size), h, tbb::simple_partitioner(), __FILE__,__LINE__);
    hiz = h.getHiz();
  }
...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
  ...
  tbb::task_scheduler_init init(atoi(argv[9]));
