...
Task_Logger::Task_Logger() {
  ...
  for (unsigned int i = 0; i < NUM_THREADS; i++) {
    std::string file_name = "sched_ov_" + std::to_string(i) + ".csv";
    report_sched[i].open(file_name);
  }  
}
...
void Task_Logger::log_sched_ov(THREADID threadid, size_t node_id, size_t work) {
  report_sched[threadid] << node_id << "," << work << std::endl;
}
...
void Task_Logger::log(THREADID threadid,
  ...
  if (last_allocated[threadid] == NUM_ENTRIES) {
    ...
    for (unsigned int i = 0; i < NUM_ENTRIES; i++) {
      //if (step_work_list[threadid][i].node_type == 1 || step_work_list[threadid][i].node_type == 2)
	//assert(step_work_list[threadid][i].work == 0);
	...
	for (std::unordered_map<size_t, size_t>::iterator it=step_work_list[threadid][i].region_work->begin();
      ...
      //step_work_list[threadid][i].work = 0;
    }
  ...
  //assert(step_work_list[threadid][last_allocated[threadid]].work == 0);
  step_work_list[threadid][last_allocated[threadid]].work += work;

  // assert(step_work_list[threadid][last_allocated[threadid]].node_type == 1 && step_work_list[threadid][last_allocated[threadid]].work == 0);
  // if(step_work_list[threadid][last_allocated[threadid]].node_type == 1) {
  //   assert(step_work_list[threadid][last_allocated[threadid]].work == 0);
  ...
  // if (step_work_list[threadid][last_allocated[threadid]].seq_num_str) {
  //   delete step_work_list[threadid][last_allocated[threadid]].seq_num_str;
  //   step_work_list[threadid][last_allocated[threadid]].seq_num_str = NULL;
  // }
  // std::string sq_str = taskGraph->getCurSeqNoStr(threadid);
  // step_work_list[threadid][last_allocated[threadid]].seq_num_str = new char[sq_str.length()+1];
  // strcpy(step_work_list[threadid][last_allocated[threadid]].seq_num_str, sq_str.c_str());
}
