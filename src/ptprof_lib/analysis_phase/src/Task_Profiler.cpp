...
size_t Task_Profiler::GenerateSchedulingOverheadProfile(std::string folder,size_t app_work) {
  ...
  //read sched_info.csv file
  // std::string file_name = folder + "/sched_info.csv";
  // std::ifstream sched_file;
  // sched_file.open(file_name);
  // if (sched_file.is_open()) {
  //   std::string line;
  //   std::getline (sched_file, line);
  //   std::stringstream sstream(line);

  //   std::string so_work;
  //   std::getline(sstream, so_work, ',');
  //   size_t shed_ohead_work = std::stoul(so_work);

  //   std::string so_calls;
  //   std::getline(sstream, so_calls, ',');
  //   size_t sched_ohead_calls = std::stoul(so_calls);

  //   std::ofstream report;
  //   report.open(folder + "/sched_ohead_profile.csv");
  //   report << "Percent scheduling overhead,Mean scheduling overhead" << std::endl;
  //   report << ((double)shed_ohead_work/(double)(app_work+shed_ohead_work))*100.00 << ","
  // 	   << (double)shed_ohead_work/(double)sched_ohead_calls
  // 	   << std::endl;
  //   report.close();
  // } else {
  //   std::cerr << "Scheduling overhead data not available\n";
  //   assert(0);
  // }
}
...
void Task_Profiler::attributeSOvToCallSite() {
  for (size_t i = 2; i <= taskGraph->last_allocated_node; i++) {
    ...
    if (task_node->type == FINISH && task_node->call_site == 0) {
      // if (taskGraph->tgraph_nodes[task_node->parent].call_site == 0) {
      // 	std::cout << task_node->parent  <<  " and type " << taskGraph->tgraph_nodes[task_node->parent].type << std::endl;
      // 	assert(taskGraph->tgraph_nodes[task_node->parent].call_site != 0);
      // }
      AFTask* callsite_parent = taskGraph->findCallsiteParent(task_node);
      callsite_parent->sc_ov_work += task_node->sc_ov_work;
      //taskGraph->tgraph_nodes[task_node->parent].sc_ov_work += task_node->sc_ov_work;
...
void Task_Profiler::report_regions_to_optimize(std::string folder, size_t mean_sov) {
  ...
  while ((double)head->t_prof.work/(double)head->t_prof.critical_child <= THRESHOLD*(NUM_PROCESSORS)) {
    ...
    //std::cout << (double)head->t_prof.work/(double)head->t_prof.critical_child << std::endl;
  }

  //Report all regions
  std::ofstream report;
  report.open(folder + "/regions_automatic.csv");
