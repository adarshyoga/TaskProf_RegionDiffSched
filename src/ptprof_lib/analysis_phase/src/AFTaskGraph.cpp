...
void AFTaskGraph::diff_anal_data (size_t total_par_work, size_t total_ser_work) {
  step_diff_file.open("step_diff_file.csv");
  ...
  step_diff_file << "Start file:line,End file:line,Work in parallel execution,Percent total work(Par),Work in serial execution,Percent total work(Ser),Percent par work in total ser work,Ratio" << std::endl;
  ...
  //print step work stats
  // for(auto it = step_stat.begin(); it != step_stat.end(); it++) {
  //   step_diff_file << (it->second).start.filename << ","
  // 		   << (it->second).start.line << ","
  // 		   << (it->second).end.filename << ","
  // 		   << (it->second).end.line << ","
  // 		   << (it->second).count
  // 		   << std::endl;
  // }
...
//   initialize_task(innode_type, call_site, parent_idx);
//   //last_allocated_node++;
//   return index;
// }

void AFTaskGraph::initialize_task (size_t index, NodeType node_type, size_t call_site, size_t parent_idx) {
  // if (tgraph_nodes[index].type != 0) {
  //   std::cout << index << std::endl;
  //   assert(tgraph_nodes[index].type == 0);
  // }
  last_allocated_node++;
...
void AFTaskGraph::create_dpst(std::string folder, bool is_parallel) {
  ...
  // report_int_nodes.open(folder + "/int_nodes.csv");
  ...
  //std::vector<struct pending_info> pending;
  ...
  for (unsigned int i = 0; i < NUM_THREADS; i++) {
    ...
    //begin_index[i] = last_allocated_node;
    //if (i == 0) last_allocated_node++; /**Temp hack to add 0th node as dummy node**/
	...
	if (static_cast<NodeType>(std::stoi(node_type)) == STEP) {
	  ...
	  //if (static_cast<NodeType>(std::stoi(node_type)) == ASYNC)
	  //assert(std::stoul(step_work_str) == 0);
	  
	  //update the sequence number string
	  ////update_seqnum_str(insert_idx, parent_index);
	  //std::string sq_str;
	  //std::getline(sstream, sq_str, ',');
	  //tgraph_nodes[insert_idx].ut_id.seq_num_str = new std::string(sq_str);
  ...
  // for (std::vector<struct pending_info>::iterator it = pending.begin(); it != pending.end();it++) {
  //   size_t parent_index = begin_index[it->parent_thd] + it->parent_idx;
  //   tgraph_nodes[it->idx].parent = parent_index;
  //   //update the number of children field in the parent index
  //   if (parent_index != 0) {
  //     tgraph_nodes[parent_index].num_children++;
  //   }
...
// void AFTaskGraph::update_seqnum_str(size_t cur_index,  size_t parent_index) {

//   if (parent_index == 0) {
//     tgraph_nodes[cur_index].ut_id.seq_num_str = new std::string("0");
//     return;
//   }
//   if (tgraph_nodes[parent_index].ut_id.seq_num_str == NULL) {
//     update_seqnum_str(parent_index, tgraph_nodes[parent_index].parent);
//   }
//   tgraph_nodes[cur_index].ut_id.seq_num_str = new std::string();
//   *(tgraph_nodes[cur_index].ut_id.seq_num_str) = *(tgraph_nodes[parent_index].ut_id.seq_num_str) + std::to_string(tgraph_nodes[parent_index].num_children);
